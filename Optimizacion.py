...
def train_model(model,dataloader,optimizer,criterion,
               ...
               log_data=False,batch_transforms=None):
  ...
  for i,sample in enumerate(dataloader):
    ...
    if batch_transforms:
      input_images,target = batch_transforms(input_images,target)
...
def eval_model(model,dataloader,criterion,
  ...
  if epoch_loss<best_loss:
    ...
    torch.save({'state_dict':model.state_dict()},"modelo.pt")
  
  if test:
    return all_labels, all_predictions,indices,all_probas
...
def test_model(model,dataloader,criterion,device=torch.device("cpu"),
              test=False):
